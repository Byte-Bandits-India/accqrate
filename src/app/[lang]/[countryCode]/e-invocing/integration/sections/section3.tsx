      ...
      <div className="max-w-[1200px] mx-auto px-6 md:px-8">
        <div>
          ...
          <FadeUp className="text-center mb-12 md:mb-16">
            <h3 className="text-[23px] lg:text-[24px] font-bold text-[#1C2041] mb-4">
          ...
          {isEUFlow && (
            <>
              {/* TOP IMAGE */}
              {countryCode === "ae" ? (
                <Reveal direction="right" className="w-full mb-12">
                  <Image
                ...
                data.heroImage && (
                  <Reveal direction="right" className="flex flex-col items-center justify-center mb-12">
                    <Image
                      src={data.heroImage}
                      width={600}
                      height={400}
                ...
                {data.steps.map((step: any, index: number) => {
                  ...
                  return (
                    <Reveal direction="left"
                      key={index}
                      className="rounded-xl overflow-hidden md:min-h-[300px] flex flex-col"
                    >
                      {/* ---------- TOP AREA ---------- */}
                      <div
                        className={`flex-1 px-6 pt-6 pb-4 ${isBlue ? "bg-[#e8eeff]" : "bg-[#f7f7f7]"
                          }`}
                      >
                        {/* ICON (CENTERED) */}
                        ...
                        {/* TEXT (LEFT ALIGNED) */}
                        <h4 className="font-bold leading-[28px] text-[16px] md:text-[18px] text-[#1C2041] mb-2">
                          <T lang={lang} countryCode={countryCode}>
                            {step.title}
                          </T>
                        </h4>
                      ...
                      {/* ---------- BOTTOM STEP BAR ---------- */}
                      <div
                        className={`px-6 py-3 text-[14px] font-medium text-[#FF6B3D] ${isBlue ? "bg-[#f0f3fb]" : "bg-[#efefef]"
                          }`}
                      >
                        <T lang={lang} countryCode={countryCode}>
                          {step.number}
